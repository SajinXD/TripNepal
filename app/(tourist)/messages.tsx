...
import { View, Text, ScrollView, Pressable, TouchableOpacity, ActivityIndicator, RefreshControl, Alert } from 'react-native';
import { useRouter } from 'expo-router';
import { ScreenHeader } from '../../src/components/layout/ScreenHeader';
import { useSafeAreaInsets } from 'react-native-safe-area-context';
import { supabase } from '../../src/lib/supabase';
import { useAuth } from '../../src/hooks/useAuth';
import { MessageSquare, Clock, CheckCircle, XCircle } from 'lucide-react-native';
  ...
  async function cancelRequest(reqId: string) {
    Alert.alert('Cancel Request', 'Cancel this connect request?', [
      { text: 'Keep', style: 'cancel' },
      {
        ...
        onPress: async () => {
          await (supabase.from('chat_requests') as any).delete().eq('id', reqId);
          loadData();
        }
      },
    ]);
  ...
  return (
    <View className="flex-1 bg-surface dark:bg-gray-900">
      ...
      <View className="px-5 pt-5 pb-3">
        <Text className="font-displayBold text-[24px] text-on-surface">Messages</Text>
      ...
      {loading ? (
        ...
        <ScrollView
          ...
          refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); loadData(); }} tintColor="#8B1A1A" />}
        >
          {/* Connect Requests */}
          {chatRequests.length > 0 && (
            <View className="px-5 pt-2 mb-2">
              <Text className="font-displaySemiBold text-[15px] text-on-surface mb-3">Connect Requests</Text>
              {chatRequests.map(req => {
                const guideName = (req.guide as any)?.full_name || 'Guide';
                ...
                return (
                  ...
                  >
                    <View className="w-10 h-10 bg-primary/10 rounded-full items-center justify-center mr-3">
                      <Text className="font-bold text-primary">{guideName[0].toUpperCase()}</Text>
                    </View>
                    <View className="flex-1">
                      <Text className="font-displaySemiBold text-[14px] text-on-surface">{guideName}</Text>
                      <View className="flex-row items-center mt-0.5">
                        {isPending && <Clock size={12} color="#D97706" />}
                        {isAccepted && <CheckCircle size={12} color="#059669" />}
                        {isRejected && <XCircle size={12} color="#DC2626" />}
                        <Text className={`text-[11px] ml-1 font-medium ${isPending ? 'text-amber-600' : isAccepted ? 'text-green-600' : 'text-red-600'}`}>
          ...
          {threads.length === 0 && chatRequests.length === 0 ? (
            <View className="items-center py-20 px-8">
              ...
              <Text className="font-displaySemiBold text-[18px] text-on-surface mb-2">No messages yet</Text>
              <Text className="font-body text-[14px] text-on-surface-variant text-center">
                Find a guide and send them a connect request to start chatting.
              </Text>
            </View>
          ) : threads.length > 0 ? (
            <View className="px-5 pt-2">
              {chatRequests.length > 0 && (
                <Text className="font-displaySemiBold text-[15px] text-on-surface mb-3">Chats</Text>
              )}
              {threads.map(thread => {
                  ...
                  >
                    ...
                    <View className="relative mr-3">
                      <View className="w-12 h-12 bg-primary/10 rounded-full items-center justify-center">
                        <Text className="font-bold text-primary text-lg">{guideInitial}</Text>
                      </View>
                    ...
                    <View className="flex-1">
                      <View className="flex-row items-center justify-between mb-1">
                        <Text className="font-displaySemiBold text-[15px] text-on-surface">{guideName}</Text>
                        <Text className="font-body text-[11px] text-on-surface-variant">{timeStr}</Text>
                      </View>
