...
import { View, Text, ScrollView, TextInput, TouchableOpacity, Alert, ActivityIndicator, KeyboardAvoidingView, Platform, Pressable } from 'react-native';
...
export default function KYCWizard() {
  ...
  function validateStep(): boolean {
    ...
    if (step === 2) {
      if (!citizenshipPath) {
        Alert.alert('Upload Required', 'Please upload your citizenship document.');
        return false;
      }
    }
    if (step === 3) {
        ...
        <ScrollView className="flex-1 px-6 pt-6" showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          ...
          {step === 1 && (
            <View>
              ...
              >
                <Text className={dateOfBirth ? 'text-text' : 'text-[#9CA3AF]'}>
                  {dateOfBirth || 'Select date of birth'}
                </Text>
              </Pressable>
              {showDOBPicker && (
                <DateTimePicker
                  value={dateOfBirth ? new Date(dateOfBirth) : new Date('1990-01-01')}
                  mode="date"
                  display="spinner"
          ...
          {step === 2 && (
            <View>
              ...
              <Text className="text-text-secondary text-sm mb-6">
                Upload your Citizenship card (front &amp; back) for identity verification.
              </Text>
              <TouchableOpacity
              ...
              <View className="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-8 flex-row">
                <Text className="text-blue-700 text-sm flex-1">
                  🪪 NTB License can be added separately in Guide Settings after verification.
                </Text>
              </View>
            </View>
          )}
