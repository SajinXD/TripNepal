...
import { CalendarDays, ChevronRight, Star, Users } from "lucide-react-native";
...
export default function GuideDashboardScreen() {
		...
		try {
			...
			const [gpRes, pendingRes, weekRes] = await Promise.all([
				(supabase.from("guide_profiles") as any)
					.select(
						"average_rating, total_reviews, total_trips_completed",
					)
					.eq("user_id", user.id)
					.single(),
			...
			const weekEarnings = (weekRes.data || []).reduce(
				(sum: number, b: any) => sum + (b.total_amount_npr || 0),
				0,
			);
			setStats({
	...
	const loadBookings = useCallback(async () => {
		...
		setActiveBookings(
			all.filter((b) => ["accepted", "in_progress"].includes(b.status)),
		);
	...
	async function respondToBooking(bookingId: string, accept: boolean) {
		...
		try {
			...
			Alert.alert(
				...
				accept
					? "Booking accepted. The tourist has been notified."
					: "Booking declined.",
			);
		} catch (e: any) {
			Alert.alert(
				"Error",
				e.message || "Failed to respond. Please try again.",
			);
		}
			...
			>
				...
				{loadingStats ? (
					...
					<View className="flex-row mb-8 gap-3">
						<Card className="flex-1 items-center py-5">
							...
							<Text className="font-bodyBold text-[10px] text-on-surface-variant uppercase tracking-wider mb-1">
								RATING
							</Text>
							...
							<Text className="font-body text-[11px] text-[#22C55E]">
				...
				<View className="flex-row justify-between items-center mb-4">
					<View className="flex-row items-center">
						...
						<Badge
							label={`${pendingBookings.length} Pending`}
							variant="terracotta"
						/>
					</View>
					<Pressable
						onPress={() => router.push("/(guide)/inbox" as any)}
					>
						<Text className="font-bodySemibold text-[14px] text-primary">
							View All
						</Text>
					</Pressable>
				</View>
				...
				{loadingBookings ? (
					...
					<Card className="mb-8 items-center py-8">
						...
						<Text className="font-semibold text-on-surface text-base mb-1">
							No pending requests
						</Text>
						<Text className="text-on-surface-variant text-sm text-center">
							Make sure your profile is verified and you're set to
							online.
						</Text>
					</Card>
				) : (
								...
								>
									...
									<View className="flex-row items-center mb-3">
										...
										<Text className="font-body text-[12px] text-on-surface-variant ml-1 flex-1">
											{booking.travelers_count} traveler
											{booking.travelers_count > 1
												? "s"
												: ""}{" "}
											·{" "}
											{booking.trip_category?.replace(
												"_",
												" ",
											)}{" "}
											· {booking.district}
				...
				{activeBookings.length > 0 && (
					<>
						<Text className="font-displaySemiBold text-[20px] text-on-surface mb-4">
							Active Bookings
						</Text>
						<View className="mb-8">
									...
									>
										<Card className="mb-3 flex-row items-center">
											<View className="w-12 items-center mr-3">
												<Text className="font-displaySemiBold text-[22px] text-on-surface">
													{day}
												</Text>
												<Text className="font-bodyBold text-[10px] text-on-surface-variant uppercase tracking-wider">
													{mon}
												</Text>
											</View>
											<View className="flex-1">
				...
				{/* Performance */}
				<Text className="font-displaySemiBold text-[16px] text-on-surface mb-4">
					Guide Performance
				</Text>
				<Card className="mb-8">
					<View className="mb-4">
						<View className="flex-row justify-between items-center mb-1">
							...
							<Text className="font-bodyBold text-[12px] text-primary">
						...
						<View className="w-full h-1.5 bg-surface-container-highest rounded-full">
							<View
								...
								style={{
									width: `${Math.min(100, ((stats?.completed_trips ?? 0) / 50) * 100)}%`,
								}}
							/>
						</View>
					</View>
					<View>
						...
						<View className="w-full h-1.5 bg-surface-container-highest rounded-full">
							<View
								...
								style={{
									width: `${Math.min(100, ((stats?.total_reviews ?? 0) / 100) * 100)}%`,
								}}
							/>
						</View>
					</View>
