...
export default function GuideInbox() {
  ...
  async function declineRequest(req: any) {
    Alert.alert('Decline Request', `Decline chat request from ${(req.tourist as any)?.full_name || 'this tourist'}?`, [
  ...
  return (
    <SafeScreen edges={['top']} bg="#F7F7F4">
      <View className="px-6 py-4 flex-row justify-between items-center bg-background border-b border-border">
        <Text className="font-display text-2xl text-text">Workspace</Text>
        {pendingCount > 0 && (
          <View className="bg-red-500 px-2.5 py-1 rounded-full">
            <Text className="text-white font-bold text-xs">{pendingCount} new</Text>
          </View>
        )}
      </View>
      ...
      >
          ...
          <View className="mb-6">
            <View className="flex-row justify-between items-center mb-3">
              <View className="flex-row items-center">
                ...
                <Text className="font-semibold text-lg text-text ml-2">Connect Requests</Text>
              </View>
              {chatRequests.length > 0 && (
                <View className="bg-red-500 px-2 py-0.5 rounded-full">
                  <Text className="text-white text-xs font-bold">{chatRequests.length}</Text>
                </View>
              )}
            </View>

            {loadingR ? (
              <View className="items-center py-4"><ActivityIndicator color="#8B1A1A" /></View>
            ) : (
              chatRequests.map(req => {
                ...
                return (
                  <View key={req.id} className="bg-card rounded-2xl border border-border p-4 shadow-sm mb-3">
                    <View className="flex-row items-center mb-3">
                      <View className="w-12 h-12 bg-primary/10 rounded-full items-center justify-center mr-3">
                        <Text className="font-bold text-primary text-base">{initial}</Text>
                      </View>
                      <View className="flex-1">
                        <Text className="font-semibold text-text">{name}</Text>
                        <Text className="text-xs text-text-secondary mt-0.5">Wants to connect with you</Text>
                      </View>
                    </View>
                    {req.message ? (
                      <Text className="text-sm text-text-secondary bg-background rounded-xl px-3 py-2 mb-3" numberOfLines={2}>{req.message}</Text>
                    ) : null}
                    <View className="flex-row gap-3">
                      ...
                      >
                        <Text className="font-semibold text-text-secondary text-sm">Decline</Text>
                      </TouchableOpacity>
                      <TouchableOpacity
                        activeOpacity={0.8}
                      ...
                      >
                        <Text className="font-semibold text-white text-sm">Accept & Chat</Text>
                      </TouchableOpacity>
                    </View>
                  </View>
                );
        ...
        <View className="mb-6">
          <View className="flex-row justify-between items-center mb-3">
            <Text className="font-semibold text-lg text-text">Booking Requests</Text>
            <TouchableOpacity onPress={() => router.push('/(guide)/dashboard' as any)}>
              <Text className="text-primary font-medium text-sm">View All</Text>
            </TouchableOpacity>
          </View>

          {loadingB ? (
            <View className="items-center py-8"><ActivityIndicator color="#8B1A1A" /></View>
          ) : bookings.length === 0 ? (
            <View className="bg-card rounded-2xl border border-border p-6 items-center">
              <Text className="text-text-secondary text-sm text-center">No booking requests yet. Make sure your profile is verified!</Text>
            </View>
          ) : (
            bookings.slice(0, 5).map(booking => {
              const cfg = STATUS_CFG[booking.status] ?? STATUS_CFG.requested;
              const touristName = (booking.tourist as any)?.full_name || 'Tourist';
              return (
                <TouchableOpacity
                  key={booking.id}
                  activeOpacity={0.7}
                ...
                >
                  <View className="flex-row items-center">
                    <View className="bg-primary/10 rounded-xl p-3 items-center justify-center w-14 h-14 mr-4">
                      <Text className="font-display text-base text-primary">{new Date(booking.start_date).getDate()}</Text>
                      <Text className="text-[9px] font-semibold text-primary uppercase">
                        {new Date(booking.start_date).toLocaleString('default', { month: 'short' })}
                      </Text>
                    </View>
                    <View className="flex-1">
                      <View className="flex-row items-center mb-1">
                        <View style={{ backgroundColor: cfg.bg }} className="px-2 py-0.5 rounded-md mr-2">
                          <Text style={{ color: cfg.color }} className="text-[9px] font-bold uppercase">{cfg.label}</Text>
                        </View>
                      </View>
                      <Text className="font-semibold text-text text-sm">{touristName}</Text>
        ...
        {/* ── Active Chats ── */}
        <View className="mb-6">
          <Text className="font-semibold text-lg text-text mb-3">Active Chats</Text>
          {loadingT ? (
            <View className="items-center py-6"><ActivityIndicator color="#8B1A1A" /></View>
          ) : threads.length === 0 ? (
            <View className="bg-card rounded-2xl border border-border p-5 items-center">
              <MessageCircle size={28} color="#9CA3AF" />
              <Text className="text-text-secondary text-sm mt-2 text-center">No active chats. Accept a booking to start chatting.</Text>
            </View>
          ) : (
            threads.map(thread => {
              ...
              const name = tourist?.full_name || 'Tourist';
              const initial = name[0].toUpperCase();
              const lastMsgTime = new Date(thread.last_message_at);
                ...
                >
                  ...
                  <View className="flex-1">
                    <View className="flex-row justify-between items-center mb-1">
                      <Text className="font-semibold text-text text-base">{name}</Text>
                      <Text className="text-xs text-text-secondary">{timeStr}</Text>
                    </View>
                    <Text className="text-sm text-text-secondary" numberOfLines={1}>Tap to open conversation</Text>
                  </View>
                  <ChevronRight size={18} color="#9CA3AF" />
                </TouchableOpacity>
              );
          ...
          <View className="bg-card rounded-2xl border border-border p-5 shadow-sm">
            <View className="mb-4">
              <View className="flex-row justify-between mb-1">
                ...
                <Text className="text-xs text-text font-bold">{stats.response}%</Text>
              </View>
              <View className="h-2 bg-background rounded-full overflow-hidden">
                <View className="h-full bg-success rounded-full" style={{ width: `${stats.response}%` }} />
              </View>
            </View>
            <View>
              <View className="flex-row justify-between mb-1">
                <Text className="text-xs text-text-secondary font-medium">Completion Rate</Text>
                <Text className="text-xs text-text font-bold">{stats.completion}%</Text>
              </View>
              <View className="h-2 bg-background rounded-full overflow-hidden">
                <View className="h-full bg-primary rounded-full" style={{ width: `${stats.completion}%` }} />
              </View>
            </View>
          </View>
        </View>
