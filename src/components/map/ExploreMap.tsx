  ...
  <script>
    ...
    destinations.forEach(function(dest) {
      ...
      marker.on('click', function(e) {
        ...
        if (window.ReactNativeWebView) {
          window.ReactNativeWebView.postMessage(JSON.stringify({ type: 'markerPress', id: dest.id }));
        } else {
    ...
    window.selectDest = function(id) {
      ...
      if (id && markers[id]) {
        ...
        map.flyTo([d.lat - 0.04, d.lng], 12, { duration: 0.5, easeLinearity: 0.5 });
      }
    };
...
export function ExploreMap({ destinations, selectedDest, setSelectedDest, mapRef, searchQuery }: any) {
  ...
  const html = buildLeafletHTML(destinations);
  ...
  useEffect(() => {
    ...
    mapRef.current = {
      fitToCoordinates: (coords: any[], options: any) => {
        const js = `window.fitToCoords(${JSON.stringify(coords)}, ${JSON.stringify(options?.edgePadding || {})}); true;`;
        webRef.current?.injectJavaScript(js);
      },
      animateToRegion: (region: any) => {
        const js = `map.flyTo([${region.latitude - 0.04}, ${region.longitude}], 12, {duration: 0.5}); true;`;
        webRef.current?.injectJavaScript(js);
      },
    };
  }, [mapRef]);
  ...
  useEffect(() => {
    const js = `window.selectDest(${selectedDest ? `'${selectedDest.id}'` : 'null'}); true;`;
    webRef.current?.injectJavaScript(js);
  }, [selectedDest?.id]);
  ...
  useEffect(() => {
    if (searchQuery !== undefined) {
      ...
      const js = `window.filterMarkers('${escaped}'); true;`;
      webRef.current?.injectJavaScript(js);
    }
  }, [searchQuery]);
  ...
  const handleMessage = (event: any) => {
    try {
      ...
      if (data.type === 'markerPress') {
        const dest = destinations.find((d: any) => d.id === data.id);
